...
export default function WaitingTime() {
  ...
  useEffect(() => {
    ...
    function animate(now: number) {
      ...
      // The arcs are the fastest, based on a 60-second cycle.
      const arcRotation = (newVirtualElapsed % 60) / 60 * 360;

      // The "minute" hand moves faster, completing a rotation every 2 virtual minutes.
      const minuteRotation = (newVirtualElapsed % (60 * 2)) / (60 * 2) * 360;

      // The "hour" hand moves 12 times slower than the "minute" hand.
      const hourRotation = minuteRotation / 12;
      

      // --- Update SVG element transforms ---
      if (hourHandRef.current) {
        hourHandRef.current.setAttribute("transform", `rotate(${hourRotation} ${CLOCK_SIZE / 2} ${CLOCK_SIZE / 2})`);
      }
      if (minuteHandRef.current) {
        minuteHandRef.current.setAttribute("transform", `rotate(${minuteRotation} ${CLOCK_SIZE / 2} ${CLOCK_SIZE / 2})`);
      }
      ...
      if (orangeArcRef.current) {
        // The orange arc is offset by 180 degrees (30 seconds)
      ...
      if (borderCircleRef.current) {
        // Rotate the black border in the opposite direction
  ...
  const blueArcPath = describeArc(CLOCK_SIZE / 2, CLOCK_SIZE / 2, ARC_RADIUS, 120, 300);
  const orangeArcPath = describeArc(CLOCK_SIZE / 2, CLOCK_SIZE / 2, ARC_RADIUS, 300, 120);
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12 w-full max-w-5xl px-4">
        ...
        >
          ...
          >
            ...
            {/* --- Clock Layers --- */}
            <circle ref={borderCircleRef} cx={CLOCK_SIZE / 2} cy={CLOCK_SIZE / 2} r={CLOCK_SIZE / 2 - 8} fill="url(#faceGlow)" stroke="#000" strokeWidth="3" />
            ...
            <rect ref={hourHandRef} x={CLOCK_SIZE / 2 - HAND_WIDTH / 2} y={CLOCK_SIZE / 2 - HAND_LENGTH + 30} width={HAND_WIDTH} height={HAND_LENGTH - 30} rx={HAND_WIDTH / 2} fill="#000" opacity="0.85" style={{ filter: "none" }} />
            <rect ref={minuteHandRef} x={CLOCK_SIZE / 2 - HAND_WIDTH / 2} y={CLOCK_SIZE / 2 - HAND_LENGTH} width={HAND_WIDTH} height={HAND_LENGTH} rx={HAND_WIDTH / 2} fill="#fdc700" opacity="0.95" />
