...
async function seedWorkers() {
    try {
        ...
        for (const workerData of testWorkers) {
            ...
            console.log(`✅ Created worker: ${worker[0].firstName} ${worker[0].lastName} (${worker[0].id})`);
        }
        ...
        for (const specData of workerSpecializations) {
            const spec = await db.insert(specializations).values(specData).returning();
            console.log(`✅ Created specialization: ${spec[0].category} - ${spec[0].subCategory} for worker ${spec[0].workerId}`);
        }
        ...
        for (const locationData of workerLocations) {
            const location = await db.insert(liveLocations).values(locationData).returning();
            console.log(`✅ Created live location: (${location[0].lat}, ${location[0].lng}) for worker ${location[0].workerId}`);
        }

        console.log('🎉 Database seeding completed successfully!');
        console.log(`📊 Created ${createdWorkers.length} workers with specializations and live locations`);
